...
def compare_models(X_train: pd.DataFrame, Y_train: pd.DataFrame) -> Dict[str, Any]:
    models = []
    ...
    pyplot.savefig("data/04_feature/compare_models.png")
    pyplot.close()
...
def predict_model(X_train: pd.DataFrame, Y_train: pd.DataFrame, X_validation: pd.DataFrame) -> pd.DataFrame:
...
def evaluate_model(Y_validation: pd.DataFrame, predictions: PickleDataSet) -> Dict[str, Any]:
